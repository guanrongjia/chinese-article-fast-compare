...
def find_words(words, num=6):
    ...
    for i in range(0, lens):
        for j in range(1, num + 1):
            if i + j < lens - num - 2:
                _words = words[i:i + j]
                if _words in split_words:
                    split_words[_words][0] += 1
                    split_words[_words][1] = float(split_words[_words][0]) / len(_words)
                    split_words[_words][6].append(words[i - 1])
                    split_words[_words][7].append(words[i + j])
                else:
                    split_words[_words] = [1,  # 出现次数
                                           1 / float(lens / len(_words)),  # TODO 这里有疑问，一个字，除以所有的字，两个字的词，除以所有字的一半，这种做法是否正确
                                           _words,  # key
                                           1,
                                           1,
                                           0,
                                           [words[i - 1]], # 左边字 # TODO 这里有可以优化的，因为左边的字不一定只有一个，可能会有两个啊三个啊，之类的
                                           [words[i + j]] # 右边字
                                       ]  # 0]
...
def find_nh(words_dic):
    for key, data_list in words_dic.items():
        # datalist 参数详解： index 1:出现频率
        if (len(key) > 1):
            # 左凝聚程度
            left_p = data_list[1] / (words_dic[key[:1]][1] * words_dic[key[1:]][1])
            # 右凝聚程度
            right_p = data_list[1] / (words_dic[key[:-1]][1] * words_dic[key[:-1]][1])
...
if __name__ == '__main__':
    # 输入文件
    words = read_file(r"C:\Users\namibox\Desktop\article-content.txt")
